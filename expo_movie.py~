...
plt_line3, = ax5.plot([], [], 'b:', label='Predicted Braking')
...
ax5.set_ylim(min(data[['pbrk', 'pred_label']].min().min(), -0.5), min(2.5, 0.5+data[['pbrk', 'pred_label']].max().max()))
ax5.set_xlim(0, (data.shape[0]-1)/fsp0)
...
#gmap.plot(data['latitude'].dropna()[0], data['longitude'].dropna()[0], 'cornflowerblue', edge_width=5)
...
def updatefig(itergen):
	...
	ax2.set_xlim(max(0,i-250), min(i+250, data.shape[0]-1))
	ax3.set_xlim(max(0,i-250), min(i+250, data.shape[0]-1))
	ax5.set_xlim(max(0,i-250), min(i+250, data.shape[0]-1))
	ax6.set_xlim(max(0,i-250), min(i+250, data.shape[0]-1))
	...
	return plt_image, plt_line, plt_line1, plt_line2, plt_line3, plt_line5, plt_line6, text, ax2, ax3, ax5, ax6, gps_image 
